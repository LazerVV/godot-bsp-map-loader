		...
		if has_geometry:
			...
			for face_idx in range(model.first_face, model.first_face + model.num_faces):
				...
				if face.surface_type == MST_PATCH:
						...
						for vx in range(tess + 1):
							...
							for j in range(h):
								for i in range(w):
									...
									if idx >= verts.size():
										print("Invalid vertex index %d for patch face %d" % [idx, face_idx])
										continue
					...
					for tess_vy in range(tess):
						for tess_vx in range(tess):
							...
							if idx11 < patch_vertices.size():
								...
								print("Invalid index calculation for patch face %d at vx=%d, vy=%d, base_idx=%d, vertex_array_size=%d" % [face_idx, tess_vx, tess_vy, b, patch_vertices.size()])
					...
					if include_patch_collision:
						# Subdivide patch into 3x3 sub-patches for collision
						for j in range(0, h - 2, 2):
							for i in range(0, w - 2, 2):
								var control: Array[Vector3] = []
								var control_uvs: Array[Vector2] = []
								var control_uv2s: Array[Vector2] = []
								var control_colors: Array[Color] = []
								for jj in range(3):
									for ii in range(3):
										...
										if idx < verts.size():
											...
											control_uvs.append(vert.uv)
											control_uv2s.append(vert.luv)
											control_colors.append(vert.color)
								if control.size() == 9:
			...
			for sh_name in ent_by_mat.keys():
				...
				if materials.has(sh_name):
					...
					print("Applied material %s to surface %d" % [sh_name, surface_idx])
				else:
		...
		if classname in TRIGGER_ENTITIES or classname in GOAL_ENTITIES:
			...
			if model:
				...
				if faces_array.size() > 0 and faces_array.size() % 3 == 0:
					...
					# Fix AABB
					debug_array_mesh.custom_aabb = AABB(model.mins, model.maxs - model.mins)
					...
					var col_shape = CollisionShape3D.new()
					col_shape.name = "TriggerShapeFallback"
					...
					var faces_array2 = PackedVector3Array()
					for idx in box_indices:
						faces_array2.append(box_vertices[idx])
					arr[Mesh.ARRAY_VERTEX] = faces_array2
					debug_array_mesh.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arr)
					var debug_mat = StandardMaterial3D.new()
					debug_mat.albedo_color = Color(1, 0, 0, 0.5)
					debug_mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
					debug_array_mesh.surface_set_material(0, debug_mat)
					debug_mesh.mesh = debug_array_mesh
					debug_mesh.name = "DebugTriggerMeshFallback"
					node.add_child(debug_mesh)
					debug_mesh.owner = root
					debug_array_mesh.custom_aabb = AABB(model.mins, model.maxs - model.mins)
					print("Added fallback box shape for entity: %s, extents: %s" % [node_name, extents])
			else:
	...
	for brush_idx in range(model.first_brush, model.first_brush + model.num_brushes):
		...
		for side_idx in range(brush.first_side, brush.first_side + brush.num_sides):
			...
			if side.plane_num < 0 or side.plane_num >= planes.size():
				...
				is_valid_brush = false
				break
			# Skip shader check for triggers, allow common/trigger
...
func convex_hull(points: PackedVector3Array) -> PackedVector3Array:
	...
	var max_points = min(100, sorted_points.size())
	for i in range(0, max_points, 3):
		if i + 2 < sorted_points.size():
			hull.append(sorted_points[i])
			hull.append(sorted_points[i + 1])
			hull.append(sorted_points[i + 2])
	return hull
